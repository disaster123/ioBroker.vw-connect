    ...
    while (this.now() < deadline) {
      ...
      try {
        ...
        if (this.logger.debug) {
          this.logger.debug("Device token poll network error; retrying: " + redactSecrets(error.message || error));
        }
        continue;
      }
      const data = response.data || {};
      if (response.status === 200) {
        if (!data.access_token) {
          throw new OAuthDeviceGrantError("Device token response did not include access_token", "invalid_token_response", response.status);
        }
        if (this.logger.debug) {
          this.logger.debug(`Device authorization succeeded (refresh token present: ${Boolean(data.refresh_token)})`);
        }
        return data;
      }
