...
function buildSeatCupraAuthorizeUrl(brand, pkce, { hybridFull = false } = {}) {
  const url = new URL(IDK_AUTHORIZE_URL);
  url.search = new URLSearchParams({
    client_id: brand.clientId,
    redirect_uri: brand.redirectUri,
    response_type: hybridFull ? "code id_token token" : "code",
    scope: brand.scope,
    state: pkce.state,
    nonce: pkce.nonce,
...
function getSeatCupraAuthStrategy(configuredStrategy) {
  if (configuredStrategy === "hybrid_full" || configuredStrategy === "device_grant") return configuredStrategy;
  return "classic_idk";
}

function getSeatCupraMissingDeviceRecoveryStrategy(currentStrategy, configuredStrategy) {
  if (currentStrategy === "hybrid_full" || configuredStrategy === "hybrid_full") return "hybrid_full";
  return "classic_idk";
}
...
function extractSeatCupraHybridCallbackTokens(location, redirectUri, expectedState) {
  if (typeof location !== "string" || !location.startsWith(redirectUri.split("://")[0] + "://")) {
    throw new SeatCupraIdkError("SEAT/CUPRA hybrid_full callback did not contain access_token/id_token");
  }
  const parsed = new URL(location);
  const sources = [
    parsed.searchParams,
  ...
  const values = {};
  for (const params of sources) {
    for (const key of ["access_token", "id_token", "code", "expires_in", "token_type", "state"]) {
      if (values[key] === undefined && params.has(key)) values[key] = params.get(key);
    }
  }
  if (values.state && expectedState && values.state !== expectedState) {
    throw new SeatCupraIdkError("SEAT/CUPRA hybrid_full callback state mismatch");
  }
  if (!values.access_token || !values.id_token) {
    throw new SeatCupraIdkError("SEAT/CUPRA hybrid_full callback did not contain access_token/id_token");
  }
  const parsedExpiry = Number.parseInt(values.expires_in, 10);
  return {
    access_token: values.access_token,
    id_token: values.id_token,
    code: values.code || "",
    expires_in: Number.isFinite(parsedExpiry) && parsedExpiry > 0 ? parsedExpiry : 7200,
...
class SeatCupraIdkAuth {
  ...
  async authenticateAuth0(loginPage, pkce, callbackHandler = (url, values) => this.exchangeCallback(url, values)) {
    ...
    return callbackHandler(callback.url, pkce);
  }

  async authenticateLegacy(loginPage, pkce, callbackHandler = (url, values) => this.exchangeCallback(url, values)) {
  ...
  async authenticateWithCallback({ hybridFull = false, callbackHandler } = {}) {
    ...
    const loginPage = await this.getAuthorizeLanding(buildSeatCupraAuthorizeUrl(this.brand, pkce, { hybridFull }));
    const handler = callbackHandler || ((url, values) => this.exchangeCallback(url, values));
    if (loginPage.url.includes("/u/login")) return this.authenticateAuth0(loginPage, pkce, handler);
    return this.authenticateLegacy(loginPage, pkce, handler);
  }
  ...
  authenticateHybridFull() {
    return this.authenticateWithCallback({
      hybridFull: true,
      callbackHandler: async (callbackUrl, pkce) => {
        const callbackTokens = extractSeatCupraHybridCallbackTokens(
          callbackUrl,
          this.brand.redirectUri,
          pkce.state,
        );
        const tokens = {
          access_token: callbackTokens.access_token,
          id_token: callbackTokens.id_token,
          refresh_token: "",
          expires_in: callbackTokens.expires_in,
          token_type: callbackTokens.token_type,
        };
        if (!callbackTokens.code) return tokens;
        try {
          const exchanged = await this.exchangeCode(callbackTokens.code, pkce.codeVerifier);
          if (exchanged.refresh_token) {
            tokens.refresh_token = exchanged.refresh_token;
            this.log("info", "SEAT/CUPRA hybrid_full code exchange captured refresh_token");
          }
        } catch {
          this.log("debug", "SEAT/CUPRA hybrid_full code exchange unavailable; using callback token");
        }
        return tokens;
      },
    });
