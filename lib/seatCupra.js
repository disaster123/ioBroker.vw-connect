...
async function runSeatCupraFreshRecovery({ login, resolveUserId }) {
  await login({ forceFreshDeviceLogin: true });
  await resolveUserId();
}
...
async function startSeatCupraDeviceAuthorization({
  ...
  if (forceFreshDeviceLogin) await clearTokens();
  return requestDeviceCode();
}

function shouldUseSeatCupraRefreshToken(forceFreshDeviceLogin, refreshToken) {
  return !forceFreshDeviceLogin && Boolean(refreshToken);
}

function beginSeatCupraMissingDeviceRecovery(authRetried, recoveryContext) {
  if (authRetried || recoveryContext.recoveryAttempted) {
