...
const SEAT_CUPRA_BRANDS = {
  seat: {
    ...
    appVersion: "2.17.0",
    idkTokenUrl: "https://ola.prod.code.seat.cloud.vwgroup.com/authorization/api/v1/token",
  },
  seatcupra: {
    ...
    appVersion: "2.15.0",
    idkTokenUrl: "https://identity.vwgroup.io/oidc/v1/token",
...
function getSeatCupraOlaHeaders(
  ...
  if (includeIdentifiers && userId) headers["User-ID"] = userId;
...
function formatSeatCupraOlaRequest(method, url, headers) {
  ...
  return (
    ...
    `userAgent=${JSON.stringify(headers["User-Agent"] || "")} auth=${headers.Authorization ? "yes" : "no"}`
  );
}
