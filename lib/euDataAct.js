...
class EuDataActClient {
  ...
  async _getBuffer(url, headers) {
    ...
    this.log.debug(
      `[euDataAct] binary GET -> ${resp.statusCode} (${size}B, ${Date.now() - startedAt}ms)`,
    );
  ...
  async downloadDataset(vin, identifier, name, _retried = false) {
    ...
    const zipMagic = buf.length > 0 && buf[0] === 0x50 && buf[1] === 0x4b; // "PK"
    const htmlResponse = /^text\/html(?:;|$)/i.test(contentType.trim());

    // Authentication responses take precedence over content classification:
    // an expired session may return an HTML login page with HTTP 401/403.
    ...
    // Adobe AEM occasionally returns a small HTML error page for an individual
    // ZIP while the authenticated metadata and list endpoints remain healthy.
    // This is a per-file delivery failure, not an authentication failure.
    if (r.status === 500 || htmlResponse) {
      const maskedVin = vin.length > 4 ? `${"*".repeat(Math.min(8, vin.length - 4))}${vin.slice(-4)}` : "****";
      this.log.debug(
        `[euDataAct] transient dataset download failure vin=${maskedVin} ` +
          `filename=${name} status=${r.status} content-type=${contentType || "unknown"} bytes=${buf.length}`,
      );
      return {
    ...
    const unzipped = unzipFirstJson(buf, name);
    return {
      ...unzipped,
      byteSize: buf.length,
      status: r.status,
